...
````
sudo mkdir -p /raidbackup/rsnapshot
````
...
snapshot_root	/raidbackup/rsnapshot/
...
backup	root@server.edu:/etc/	backupetc/
backup	root@server.edu:/export/	backupexport/
````
...
Check the weekly.0 directory:

ls /rsnapbackup/weekly.0/
````
...
0 21 * * 5 [ $(date +\%d) -le 07 ] && /usr/bin/rsnapshot weekly
0 21 * * 5 [ $(date +\%d) -le 07 ] && /usr/bin/rsnapshot monthly
````

https://www.ostechnix.com/setup-backup-server-using-rsnapshot-linux/
